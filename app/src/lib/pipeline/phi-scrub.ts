...
/**
 * Convenience wrapper used by the legacy Deepgram code path (no longer called
 * in the Gemini path — Gemini handles contextual scrubbing internally).
 *
 * Retained so the old `scrubTranscript(text, apiKey)` call site still compiles
 * during the transition; the apiKey param is accepted but unused.
 */
export async function scrubTranscript(
  transcript: string
): Promise<{ clean: string; totalRedactions: number }> {
  const result = regexScrub(transcript);
  const totalRedactions = result.redactions.reduce((s, r) => s + r.count, 0);
  return { clean: result.text, totalRedactions };
}
