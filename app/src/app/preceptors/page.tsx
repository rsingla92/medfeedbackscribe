...
interface Preceptor {
  ...
  site: string | null;
  /**
   * Null for shared/seeded institutional rows (not editable by residents),
   * non-null for preceptors a resident created themselves. See migration 007.
   */
...
export default function PreceptorsPage() {
  ...
  // Supplied by GET /api/preceptors so we can hide edit/delete buttons on
  // rows the current user doesn't own. Server-side ownership checks are the
  // security boundary; this is UX only.
          ...
          <div className="space-y-2">
            {preceptors.map((p) => (
              ...
              >
                ...
                {/* Actions — hidden for rows the user doesn't own. Seed /
                   shared preceptors have created_by_user_id === null and
                   can't be mutated by any resident (enforced server-side by
                   the UPDATE/DELETE WHERE clauses). */}
